...
def main():
    ...
    parser.add_argument('--output', dest='output', required=False, help='Path to save the plot as a PNG file')
    parser.add_argument('--html_output', dest='html_output', required=False, help='Path to save the plot as an HTML file')
    ...
    # Step 4: Add jitter to PC1, PC2, and PC3 values
    ...
    pcoa_df['PC3'] += np.random.uniform(-jitter_strength, jitter_strength, size=len(pcoa_df))
    ...
    # Plot using Plotly for 3D scatter plot
    fig = px.scatter_3d(pcoa_df, x='PC1', y='PC2', z='PC3', text='Sample', color='Sample',
                        title=f'3D PCoA of Bray-Curtis Dissimilarity\nPC1: {pc1_variance:.2f}% variance, PC2: {pc2_variance:.2f}% variance, PC3: {pc3_variance:.2f}% variance',
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            zaxis_title=f'PC3 ({pc3_variance:.2f}%)',
        ),
        width=1000,   # Slightly larger plot width
        height=800    # Slightly larger plot height
    )

    # If the output file path is provided, save the plot as PNG
    if args.output:
        fig.write_image(args.output)
        print(f"Plot saved as {args.output}")

    # If the HTML output file path is provided, save the plot as HTML
    if args.html_output:
        fig.write_html(args.html_output)
        print(f"Interactive plot saved as {args.html_output}")

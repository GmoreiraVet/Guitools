...
def natural_sort(file_paths):
    """
    Sorts file paths in a natural order based on numerical prefixes.
    """
...
def get_custom_sample_names(original_names):
    ...
    """
    print("\nOriginal sample names detected:")
...
def load_bracken_files(input_folder, rank="G", top_n=15):
    ...
    if not file_paths:
        print("Error: No Bracken files found in the specified folder.")
        return pd.DataFrame()
    ...
    combined_df_with_other = pd.concat([top_taxa_df, other_taxa_sum], ignore_index=True)
